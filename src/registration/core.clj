...
(defn process-cell [cell]
  (let [value (.getStringCellValue cell)]
    (cond
      ...
      (re-matches #"(?i).*PM.*" value) :vacation
...
(def column-header-values ["Data" "" "Hora Entrada"  "Saida Almoço" "Assinatura" "Entrada Almoço" "Hora Saida" "Assinatura"])
...
(defn -main [& args]
  (let [[month year name vacation signature output] args]
